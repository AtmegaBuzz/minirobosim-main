...
class MyRover(Rover):
	...
	# Perception
		...
		right_data = np.array(self.laserscan.values[  :15])
		center_data = np.array(self.laserscan.values[15:30])
		left_data = np.array(self.laserscan.values[30:  ])
		right = np.average(right_data)
		center = np.average(center_data)
		left = np.average(left_data)
		...
		v_l = motor_control[0] - motor_control[1]
		v_r = motor_control[0] + motor_control[1]
...
if __name__ == "__main__":
	...
	world.bodies.append( Ball( world, position=(+95,+70)) )
